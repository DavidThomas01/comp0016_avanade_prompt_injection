  ...
  return (
    <div className="min-h-[calc(100vh-64px)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
        {/* Hero */}
        <div className="glass-strong rounded-3xl p-7 sm:p-10 border border-white/60 shadow-sm">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
            ...
            <div className="grid grid-cols-2 gap-3 w-full max-w-md">
              <div className="glass rounded-2xl p-4">
                <div className="h-9 w-9 rounded-xl bg-orange-500/10 flex items-center justify-center mb-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {vulnerabilities.map((vuln) => (
              <Link
                ...
                className="glass rounded-2xl p-5 border border-white/60 hover:bg-white/80 hover:shadow-md hover:-translate-y-0.5 transition-all group focus-ring"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="font-semibold text-gray-900">{vuln.name}</div>
                        <div className="text-xs text-gray-500">{vuln.code}</div>
                      </div>
                    </div>
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      {vuln.tags.slice(0, 3).map((tag) => (
                        <span
                          key={tag}
                          className="text-[11px] px-2 py-1 rounded-full bg-gray-900/5 text-gray-700 border border-gray-900/10"
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  <ArrowRight className="h-5 w-5 text-gray-400 group-hover:text-orange-600 transition-colors mt-1" />
                </div>
              </Link>
            ))}
          </div>
